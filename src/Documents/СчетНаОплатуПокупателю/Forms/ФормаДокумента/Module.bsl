...
Процедура ПеренестиВТоварыИУслугиВместоНаСервере()
	...
	Для каждого стр Из Объект.Товары Цикл
		...
		// пересчитаем сумму НДС, поскольку ТоварыНоменклатураПриИзмененииНаСервере заполняет сумму и цену 
		...
		Иначе
			стр.СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(
				...
				УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(ДанныеСтрокаТаблицы.СтавкаНДС, Ложь)
			);
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(стр, ДанныеСтрокаТаблицы, "СтавкаНДС, СуммаНДС, ЭтоУслуга");
