...
&НаКлиенте
Процедура ЗаполнитьВыставляемыеРаботы(Команда)
...
Процедура ЗаполнитьВыставляемыеРаботыСервер()
	...
	Запрос.Текст=
	"ВЫБРАТЬ
	|	bz_ВыставленныеРаботыОстатки.Проект КАК Проект,
	...
	|	bz_ВыставленныеРаботыОстатки.Документ КАК ПервичныйДокумент,
	|	bz_ВыставленныеРаботыОстатки.НомерСтр КАК НомерСтрокиПервичногоДокумента,
	|	bz_ВыставленныеРаботыОстатки.КоличествоКВыставлениюОстаток КАК Количество,
	|	bz_ВыставленныеРаботыОстатки.СуммаКВыставлениюОстаток КАК Сумма
	|ИЗ
	|	РегистрНакопления.bz_ВыставленныеРаботы.Остатки(
	|			&Дата,
	|			Организация = &Организация) КАК bz_ВыставленныеРаботыОстатки";
