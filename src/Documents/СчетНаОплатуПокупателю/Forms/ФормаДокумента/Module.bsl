...
Асинх Процедура ЗаполнитьВыставляемыеРаботы(Команда)
	...
	Если Объект.Проведен Тогда
		...
		Возврат;
	КонецЕсли;
	Если Объект.bz_ВыставляемыеРаботы.Количество() Тогда
...
Процедура ПеренестиВТоварыИУслугиВместоНаСервере()
	...
	// Кусок ниже скопирован из Процедура ТоварыНоменклатураПриИзмененииНаКлиенте(СтрокаТаблицы)
	// и переделан под заполнение строк в цикле. 
	// Первые 2 строки - ДанныеСтрокаТаблицы = Новый Структура( и ЗаполнитьЗначенияСвойств(ДанныеСтрокаТаблицы  вынесены в цикл		
	#Область СкопированоИзТоварыНоменклатураПриИзмененииНаКлиенте
	...
	Для каждого стр Из Объект.Товары Цикл
		// заполняем представление номенклатуры - вычисляемое поле формы ТЧ Товары, используемое для отображения.
		стр.НоменклатураИнтерактивная = стр.Номенклатура;
		
		// копируем первые 2 строки из Процедура ТоварыНоменклатураПриИзмененииНаКлиенте(СтрокаТаблицы)
		СтрокаТаблицы = стр;		
		#Область СкопированоИзТоварыНоменклатураПриИзмененииНаКлиенте
		...
		// теперь копируем из другой части ф-ии - строка после ПараметрыОбъекта.ПокупательНалоговыйАгентПоНДС = ПокупательНалоговыйАгентПоНДС;
		// А во второй строчке добавляем исключения - колонки цены, количества, сумм и скидок. Они заполняются из ТЧ Работ		
		#Область СкопированоИзТоварыНоменклатураПриИзмененииНаКлиенте
		ТоварыНоменклатураПриИзмененииНаСервере(ДанныеСтрокаТаблицы, ПараметрыОбъекта);
		
		#Область ИзмененТиповойКод
		//ЗаполнитьЗначенияСвойств(СтрокаТаблицы, ДанныеСтрокаТаблицы);
		ЗаполнитьЗначенияСвойств(СтрокаТаблицы, ДанныеСтрокаТаблицы,, "Количество, Цена, Сумма, СуммаНДС, СуммаСкидки, ПроцентСкидки");
		#КонецОбласти
		#Область Добавлено
		Если ПокупательНалоговыйАгентПоНДС = Истина И ВедетсяУчетНДСПоФЗ335 = Истина Тогда 
			стр.СуммаНДС = 0;
		Иначе
			стр.СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(стр.Сумма, Объект.СуммаВключаетНДС, УчетНДСВызовСервераПовтИсп.ПолучитьСтавкуНДС(стр.СтавкаНДС, Ложь));
		КонецЕсли;
		#КонецОбласти
